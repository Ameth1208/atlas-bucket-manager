...
describe('CreateBucketUseCase', () => {
  ...
  describe('Validation', () => {
    it('should throw error if providerId is missing', async () => {
      await expect(
        useCase.execute({ providerId: '', name: 'valid-bucket' })
      ).rejects.toThrow('Provider ID and bucket name are required');
    ...
    it('should throw error if bucket name is missing', async () => {
      await expect(
        useCase.execute({ providerId: 'minio', name: '' })
      ).rejects.toThrow('Provider ID and bucket name are required');
    ...
    it('should throw error if bucket name is too long', async () => {
      ...
      await expect(
        useCase.execute({ providerId: 'minio', name: longName })
      ).rejects.toThrow('Bucket name must be between 3 and 63 characters');
    ...
    it('should throw error if bucket name has invalid characters', async () => {
      await expect(
        useCase.execute({ providerId: 'minio', name: 'Invalid_Bucket!' })
      ).rejects.toThrow('Bucket name must follow S3 naming conventions');
    ...
    it('should throw error if bucket name starts with hyphen', async () => {
      await expect(
        useCase.execute({ providerId: 'minio', name: '-invalid' })
      ).rejects.toThrow('Bucket name must follow S3 naming conventions');
  ...
  describe('Error Handling', () => {
    it('should propagate repository errors', async () => {
      ...
      await expect(
        useCase.execute({ providerId: 'minio', name: 'test-bucket' })
