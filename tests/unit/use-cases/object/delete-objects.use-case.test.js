...
const delete_objects_use_case_1 = require("../../../../src/application/use-cases/object/delete-objects.use-case");
...
describe('DeleteObjectsUseCase', () => {
    ...
    beforeEach(() => {
        ...
        useCase = new delete_objects_use_case_1.DeleteObjectsUseCase(mockRepository);
    });
    describe('Validation', () => {
        it('should throw error if objectNames is not an array', async () => {
            await expect(useCase.execute('minio', 'test-bucket', 'not-an-array')).rejects.toThrow('Object names must be a non-empty array');
            expect(mockRepository.deleteObjects).not.toHaveBeenCalled();
        });
        it('should throw error if objectNames is empty array', async () => {
            await expect(useCase.execute('minio', 'test-bucket', [])).rejects.toThrow('Object names must be a non-empty array');
            expect(mockRepository.deleteObjects).not.toHaveBeenCalled();
        });
    });
    describe('Success Cases', () => {
        it('should delete single object', async () => {
            ...
            expect(mockRepository.deleteObjects).toHaveBeenCalledWith('minio', 'test-bucket', ['file.txt']);
        ...
        it('should delete multiple objects', async () => {
            ...
            expect(mockRepository.deleteObjects).toHaveBeenCalledWith('aws', 'my-bucket', objectNames);
        });
        it('should handle objects with special characters', async () => {
            ...
            expect(mockRepository.deleteObjects).toHaveBeenCalledWith('minio', 'test-bucket', objectNames);
        });
    });
    describe('Error Handling', () => {
        it('should propagate repository errors', async () => {
            ...
            await expect(useCase.execute('minio', 'test-bucket', ['file.txt'])).rejects.toThrow('Permission denied');
        });
    });
});
//# sourceMappingURL=delete-objects.use-case.test.js.map